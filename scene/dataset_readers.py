...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        # load object masks
        # first load default masks
        default_mask_folder = images_folder.replace("images", "multiview_masks_default_merged")
        default_masks = []
        default_folders = sorted(os.listdir(default_mask_folder))
        num_default_object = len(default_folders)
        for obj_id in range(num_default_object):
            mask_path = os.path.join(default_mask_folder, f"{obj_id:03d}", f"{image_name}.jpg")
            if os.path.exists(mask_path):
                mask = Image.open(mask_path)
            else:
                continue
            default_masks.append(mask)
        # then load middle masks
        middle_mask_folder = images_folder.replace("images", "multiview_masks_middle_merged")
        middle_masks = []
        middle_folders = sorted(os.listdir(middle_mask_folder))
        num_middle_object = len(middle_folders)
        for obj_id in range(num_middle_object):
            mask_path = os.path.join(middle_mask_folder, f"{obj_id:03d}", f"{image_name}.jpg")
            if os.path.exists(mask_path):
                mask = Image.open(mask_path)
            else:
                continue
            middle_masks.append(mask)
        # then load small masks
        small_mask_folder = images_folder.replace("images", "multiview_masks_small_merged")
        small_masks = []
        small_folders = sorted(os.listdir(small_mask_folder))
        num_small_object = len(small_folders)
        for obj_id in range(num_small_object):
            mask_path = os.path.join(small_mask_folder, f"{obj_id:03d}", f"{image_name}.jpg")
            if os.path.exists(mask_path):
                mask = Image.open(mask_path).convert("L")
                # binary mask
                mask = mask.point(lambda p: p > 128)
            else:
                continue
            small_masks.append(mask)
        
        if "cam" in image_name: # for Neu3D dataset
            fid = int(image_name.split("_")[0]) / (num_frames - 1)
        elif "DSC" in image_name: # for MipNeRF360 dataset
            fid = int(image_name.strip("_DSC")[1]) / (num_frames - 1)
        else:
            fid = int(image_name.split("_")[-1]) / (num_frames - 1)
    ...
    if eval:
        if os.path.exists(os.path.join(path, "test.txt")):
            with open(os.path.join(path, "test.txt")) as f:
                ...
                test_names = [x.rstrip('\n').rstrip(' ').rstrip(".jpg").rstrip(".JPG") for x in test_names]
            # test_indices = [int(x.rstrip('\n')) for x in test_indices]
            test_cam_infos = [c for idx, c in enumerate(cam_infos) if c.image_name in test_names]
            if os.path.exists(os.path.join(path, "train.txt")):
                with open(os.path.join(path, "train.txt")) as f:
                    train_names = f.readlines()
                    train_names = [x.rstrip('\n').rstrip(' ').rstrip(".jpg") for x in train_names]
                # train_indices = [int(x.rstrip('\n')) for x in train_indices]
                train_cam_infos = [c for idx, c in enumerate(cam_infos) if c.image_name in train_names]
            else:
                train_cam_infos = [c for idx, c in enumerate(cam_infos) if c.image_name not in test_names]
