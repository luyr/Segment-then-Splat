...
class Scene:
    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, load_iteration=None, shuffle=True,
        ...
        if self.loaded_iter:
            print(f"loading from iter {self.loaded_iter}")
            self.gaussians.load_ply(os.path.join(self.model_path,
                                                 "point_cloud",
                                                 "iteration_" + str(self.loaded_iter),
                                                 "point_cloud.ply"),
                                    og_number_points=len(scene_info.point_cloud.points))
            self.gaussians.default_object_id = torch.load(f"{self.model_path}/default_object_id_{self.loaded_iter}.pth")
            self.gaussians.middle_object_id = torch.load(f"{self.model_path}/middle_object_id_{self.loaded_iter}.pth")
            self.gaussians.small_object_id = torch.load(f"{self.model_path}/small_object_id_{self.loaded_iter}.pth")
            # self.gaussians.default_object_id = torch.load(f"{self.model_path}/object_id_20000.pth")
        else:
