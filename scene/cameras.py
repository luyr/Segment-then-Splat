...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask, image_name, uid,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device="cuda", fid=None, depth=None, object_masks=None):
        ...
        # object supervision for each object
        if object_masks:
            self.default_object_masks = [torch.Tensor(mask) for mask in object_masks['default']]
            self.middle_object_masks = [torch.Tensor(mask) for mask in object_masks['middle']]
            self.small_object_masks = [torch.Tensor(mask) for mask in object_masks['small']]
            self.object_masks = {'default': self.default_object_masks, 'middle': self.middle_object_masks,
                                 'small': self.small_object_masks}
        else:
