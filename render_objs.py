...
def render_set(model_path, load2gpu_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, deform, is_deform, obj_id, obj_level):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        if is_deform:
            ...
            d_xyz = d_rotation = d_scaling = 0.0
        results = render(view, gaussians, pipeline, background, d_xyz, d_rotation, d_scaling, is_6dof, obj_level=obj_level, obj_id=obj_id)
...
def interpolate_view_original_obj(model_path, load2gpt_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background,
                              timer, is_deform=False, obj_id=None, obj_level=None):
    render_path = os.path.join(model_path, name, "interpolate_hyper_view_{}".format(iteration), "renders")
    depth_path = os.path.join(model_path, name, "interpolate_hyper_view_{}".format(iteration), "depth")
    # acc_path = os.path.join(model_path, name, "interpolate_all_{}".format(iteration), "acc")
...
def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, skip_train: bool, skip_test: bool,
                mode: str, obj_id=None, obj_level=None):
    with torch.no_grad():
        ...
        if not skip_train:
            render_func(dataset.model_path, dataset.load2gpu_on_the_fly, dataset.is_6dof, f"train_{obj_level}_{obj_id:03d}", scene.loaded_iter,
                        scene.getTrainCameras(), gaussians, pipeline,
                        background, deform, dataset.deform, obj_id, obj_level)

        if not skip_test:
            render_func(dataset.model_path, dataset.load2gpu_on_the_fly, dataset.is_6dof, f"test_{obj_level}_{obj_id:03d}", scene.loaded_iter,
                        scene.getTestCameras(), gaussians, pipeline,
                        background, deform, dataset.deform, obj_id, obj_level)

def render_objs(dataset: ModelParams, iteration: int, pipeline: PipelineParams, skip_train: bool, skip_test: bool,
                mode: str):
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree)
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False)
        ...
        default_obj_ids = torch.unique(gaussians.default_object_id)
        default_obj_ids = default_obj_ids[default_obj_ids != 255]
        largest_id = default_obj_ids.max()
        default_obj_ids = torch.arange(largest_id+1)
        
        middle_obj_ids = torch.unique(gaussians.middle_object_id)
        middle_obj_ids = middle_obj_ids[middle_obj_ids != 255]
        largest_id = middle_obj_ids.max()
        middle_obj_ids = torch.arange(largest_id+1)
        
        try:
            small_obj_ids = torch.unique(gaussians.small_object_id)
            small_obj_ids = small_obj_ids[small_obj_ids != 255]
            largest_id = small_obj_ids.max()
            small_obj_ids = torch.arange(largest_id+1)
        except:
            small_obj_ids = torch.tensor([])
        ...
        if not skip_train:
            ...
            render_func(dataset.model_path, dataset.load2gpu_on_the_fly, dataset.is_6dof, f"train", scene.loaded_iter,
                                scene.getTrainCameras(), gaussians, pipeline, 
                                background, deform, dataset.deform, None, None)
            for obj_level in ['default', 'middle', 'small']:
                for obj_id in obj_id_dict[obj_level]:
                    try:
                        render_func(dataset.model_path, dataset.load2gpu_on_the_fly, dataset.is_6dof, f"train_{obj_level}_{obj_id:03d}", scene.loaded_iter,
                                    scene.getTrainCameras(), gaussians, pipeline,
                                    background, deform, dataset.deform, obj_id, obj_level)
        ...
        if not skip_test:
            # first render entire scene
            render_func(dataset.model_path, dataset.load2gpu_on_the_fly, dataset.is_6dof, f"test", scene.loaded_iter,
                                scene.getTestCameras(), gaussians, pipeline, 
                                background, deform, dataset.deform, None, None)
            for obj_level in ['default', 'middle', 'small']:
                for obj_id in obj_id_dict[obj_level]:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--obj_id", default=None, type=int)
    parser.add_argument("--obj_level", default='default', choices=['default', 'middle', 'small'])
    ...
    render_objs(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.mode)
